...
def test_add_fingerprint(fingerprint, name):
    ...
    data = {
        'name': name,
        'fingerprint': fingerprint  # Отправляем массив из 128 чисел
    }
...
def test_compare_fingerprint(fingerprint):
    """Тест для API сравнения отпечатков пальцев."""
    ...
    if response.status_code == 200:
        print("Отпечаток пальца распознан:")
        print(response.json())
...
if __name__ == '__main__':
    ...
    test_fingerprint = [0.1] * 128  # Пример массива из 128 чисел
    test_add_fingerprint(test_fingerprint, 'John Fingerprint')

    # Тестирование повторного добавления уже существующего участника
    print("\nТестирование добавления уже существующего участника:")
    test_add_fingerprint(test_fingerprint, 'John Fingerprint')

    # Тестирование сравнения отпечатка пальца
    print("\nТестирование сравнения отпечатка пальца:")
    test_compare_fingerprint(test_fingerprint)
